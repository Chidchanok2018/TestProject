...
# -------------- รอบที่ 4 -----------------------
# Next_SubN2N1 ต้องการ (list[.,.,.], list[set([],[])])
S3N2_R4 = Next_SubN2N1(S3N2_inter_R3, Sub3)  # หาโหนดใกล้เคียงเหมือนกัน 2 โหนด(3) list[set([],[])]
print'Sub จากการหาโหนดเหมือนกัน 2 รอบที่ 5 =', len(S3N2_R4), 'Cycles'
S3N2_inter_R4 = interintra2(S3N2_inter_R3, S3N2_R4)  # หาโหนดที่มี DifDen มากกว่าเท่ากับ 0.70 list[.,.,.]

if len(S3N2_R2) or len(S3N2_R3) or len(S3N2_R3) == 0:  # เมื่อไม่เจอ sub รอบๆแล้ว ให้มาขึ้น Cluster ใหม่
    ...
    print'Rest_S3N2_inter_R3 =', Rest_S3N2_inter_R3

    if len(Rest_S3N2_R1) or len(Rest_S3N2_R2) or len(Rest_S3N2_R3) == 0:  # เมื่อไม่เจอ sub รอบๆแล้ว ให้มาขึ้น Cluster ใหม่
        Cluster2 = Rest_S3N2_inter_R2  # list[.,.,.] นับไม่ได้ ไว้สร้างกราฟ
        print'โหนดทั้งหมดที่ใช้ในการหาครั้ง 2 =', Cluster2
        P1 = len(Rest_S3N2_R0) + len(Rest_S3N2_R1) + len(Rest_S3N2_R2) + len(Rest_S3N2_R3)
        ...
        if len(Rest1_S3N2_R1) or len(Rest1_S3N2_R2) or Rest1_S3N2_R3 == 0:  # เมื่อไม่เจอ sub รอบๆแล้ว ให้มาขึ้น Cluster ใหม่
            ...
            if len(Rest2_S3N2_R1) or len(Rest2_S3N2_R2) == 0:  # เมื่อไม่เจอ sub รอบๆแล้ว ให้มาขึ้น Cluster ใหม่
                ...
                Rest3_S3N21_inter_R2 = interintra2(Rest3_S3N2_inter_R1, Rest3_S3N2_R2)  # คำนวน รnterintra รอบที่ 3

                if Rest3_S3N2_inter_R1 == None:
                    print'Sub จากการหาโหนดเหมือนกัน 2 รอบที่ 5 = ไม่มีโหนดเหลือ'
...
# -------------- รอบที่ 1 -----------------------
C2_S3N2_R0 = CutSub(ClusterA, Sub3)  # หา Sub ที่เหลือจาก Cluster1
print'Sub ที่เหลือจากการใช้ครั้ง 2 =', C2_S3N2_R0
print'จำนวน Sub ที่เหลือจากการใช้ครั้ง 2 =', len(C2_S3N2_R0), 'Cycles'
C2_S3N2_Sorted = sorted(C2_S3N2_R0)  # จัดเรียงซับที่เหลือจาก Cluster1
C2_S3N2_inter_R0 = interintra(C2_S3N2_R0, C2_S3N2_Sorted)  # เอาซับที่เหลือหา interintra
# -------------- รอบที่ 2 -----------------------
# Next_SubN2N1 ต้องการ (list[.,.,.], list[set([],[])])
C2_S3N2_R1 = Next_SubN2N1(C2_S3N2_inter_R0, Sub3)  # หาโหนดใกล้เคียงเหมือนกัน 2 โหนด list[set([],[])]
print'Sub จากการหาโหนดเหมือนกัน 2 รอบที่ 2 =', len(C2_S3N2_R1), 'Cycles'
C2_S3N2_inter_R1 = interintra2(C2_S3N2_inter_R0, C2_S3N2_R1)  # หาโหนดที่มี DifDen มากกว่าเท่ากับ 0.70 list[.,.,.]
#print'C2_S3N2_inter_R1', C2_S3N2_inter_R1
# -------------- รอบที่ 3 -----------------------
# Next_SubN2N1 ต้องการ (list[.,.,.], list[set([],[])])
C2_S3N2_R2 = Next_SubN2N1(C2_S3N2_inter_R1, Sub3)  # หาโหนดใกล้เคียงเหมือนกัน 2 โหนด(2) list[set([],[])]
print'Sub จากการหาโหนดเหมือนกัน 2 รอบที่ 3 =', len(C2_S3N2_R2), 'Cycles'
# C2_S3N2_R2 = 0
print'C2_inter_R1 =', C2_S3N2_inter_R1 # list สร้างกราฟได้
...
Cluster3_M = Cluster3
Cluster4_M = Cluster4
C2_M = C2_S3N2_inter_R1
Node_BetweenC = FindNodesBetweenCluster(Cluster1_M, Cluster2_M, Cluster3_M, Cluster4_M, C2_M, float(len(G.edges)))
# --------------Draw Graph-----------------
G = nx.Graph()
