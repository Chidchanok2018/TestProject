...
def interintra21(N_C21, N_C2_I_L, N_C2_I, DD):  # list[['','',''],['','','']], list[[....]]
    Result = N_C2_I  # ก้อนสั้นของครัสเตอร์รอบที่แล้ว แล้วต้องนำไป append ด้วยก้อนสั้นที่คะนวนแล้ว
    N_C21  # ก้อนสั้นของซับข้างเคียงที่จะเอามาคำนวน
    N_C2_I_L  # ก้อนยาวที่คำนวนจากครั้งแรกแล้ว ก้อนยาว ๆ ซ้ำ ๆ
    Start = N_C2_I_L  # list
    Keep = []
    count = len(N_C21)
    for h in range(count - 1):
        ...
        if len(b) >= 2:
            ...
            # Source = h + 1.00  # จำนวนรอบหมุนเพิ่ม 1
            Number_of_Edes_All = float(len(G.edges(a)))  # จำนวนกิ่งภายในครัสเตอร์
            N = float(len(G.nodes(a)))  # จำนวนโหนดทั้งหมด
            N_C = float(len(G.nodes(a)))    # จำนวนโหนดภายในครัสเตอร์
...
N3_C2_I = interintra(N_C2, N_C2_S, 0.80)  # tuple [[Subชุดแรกที่ผ่านการต่อไซเคิล]]
...
N_C21_I = interintra21(N_C21, N_C2_I_Llist, N3_C2_I, 0.80)
