...
def FindNodesBetweenCluster(ClusterS, ClusterC, ClusterC1, ClusterC2, C2, Float_Edges_All):
    G = nx.Graph()
    ClusterA = ClusterS + ClusterC + ClusterC1 + ClusterC2 # กราฟรวมมีกิ่งเชื่อม
    ClusterB = C2
    N_Edges_All = Float_Edges_All
    ...
    G.add_cycle(ClusterB)
    a = G.edges(ClusterA)  #EdgesDataView
    print'จำนวนกิ่งใน Cluster 1 =', len(a)
    b = G.edges(ClusterB)  #EdgesDataView
    print'จำนวนกิ่งใน Cluster 2 =', len(b)
    print'จำนวนกิ่งในครัสเตอร์ทั้งหมด =', float(len(a)) + float(len(b))
    print'จำนวนกิ่งในกราฟทั้งหมด =', N_Edges_All
    print'Solution Coverage Matric'
    N_intra_Edges = float(len(a)) + float(len(b))
    Coverage = N_intra_Edges / N_Edges_All
    print'= ', '%-10.2f' % Coverage

    draw_networkx(G, edge_color='b')
    plt.savefig('Snowball4_Test1')
    plt.figure(1)
    plt.show()
    return ClusterA
...
print'Sub จากการหาโหนดเหมือนกัน 2 รอบที่ 3 = ไม่ผ่าน inter-intra'

if len(S3N2_R1) or len(S3N2_R2) == 0:  # เมื่อไม่เจอ sub รอบๆแล้ว ให้มาขึ้น Cluster ใหม่
    Cluster1 = S3N2_inter_R1  # list[.,.,.] นับไม่ได้ ไว้สร้างกราฟ
    print'โหนดทั้งหมดที่ใช้ในการหาครั้งแรก =', Cluster1
    print'จำนวนโหนดที่มีในครั้งแรก =', len(S3N2_R0) + len(S3N2_R1) + len(S3N2_R2)
    P = len(S3N2_R0) + len(S3N2_R1) + len(S3N2_R2)
    Q = Number_of_nodes - P
    print'จำนวนโหนดที่เหลืออยู่ในกราฟ=', Q, 'โหนด'

    print'----เริ่มการหา Sub ข้างเคียงรอบที่ 2---------------'
    # -------------- รอบที่ 1 -----------------------
    Rest_S3N2_R0 = CutSub(S3N2_inter_R1, Sub3)  # หา Sub ที่เหลือจาก Cluster1
    ...
    if len(Rest_S3N2_R1) or len(Rest_S3N2_R2) or len(Rest_S3N2_R3) or len(Rest_S3N2_R4) == 0:  # เมื่อไม่เจอ sub รอบๆแล้ว ให้มาขึ้น Cluster ใหม่
        Cluster2 = Rest_S3N2_inter_R3  # list[.,.,.] นับไม่ได้ ไว้สร้างกราฟ
        print'โหนดทั้งหมดที่ใช้ในการหาครั้ง 2 =', Cluster2
        P1 = len(Rest_S3N2_R0) + len(Rest_S3N2_R1) + len(Rest_S3N2_R2) + len(Rest_S3N2_R3) + len(Rest_S3N2_R4)
        ...
        Rest1_S3N2_inter_R2 = interintra2(Rest1_S3N2_inter_R1, Rest1_S3N2_R2)  # คำนวน รnterintra รอบที่ 3
        # -------------- รอบที่ 4 -----------------------
        Rest1_S3N2_R3 = Next_SubN2N1(Rest1_S3N2_inter_R2, Sub3)  # หา Sub ข้างเคียงใหม่อีกครั้ง รอบที่ 4
        print'Sub จากการหาโหนดเหมือนกัน 2 รอบที่ 3=', len(Rest1_S3N2_R3)  # พิมพ์จำนวน sub ข้างเคียงรอบที่ 4
        Rest1_S3N2_inter_R3 = interintra2(Rest1_S3N2_inter_R2, Rest1_S3N2_R3)  # คำนวน รnterintra รอบที่ 4
        # -------------- รอบที่ 5 -----------------------
        Rest1_S3N2_R4 = Next_SubN2N1(Rest1_S3N2_inter_R3, Sub3)  # หา Sub ข้างเคียงใหม่อีกครั้ง รอบที่ 5
        print'Sub จากการหาโหนดเหมือนกัน 2 รอบที่ 4=', len(Rest1_S3N2_R4)  # พิมพ์จำนวน sub ข้างเคียงรอบที่ 5
        Rest1_S3N2_inter_R4 = interintra2(Rest1_S3N2_inter_R3, Rest1_S3N2_R4)  # คำนวน รnterintra รอบที่ 5
        ...
        if len(Rest1_S3N2_R1) or len(Rest1_S3N2_R2) == 0:  # เมื่อไม่เจอ sub รอบๆแล้ว ให้มาขึ้น Cluster ใหม่
            ...
            P2 = len(Rest1_S3N2_R0) + len(Rest1_S3N2_R1) + len(Rest1_S3N2_R2) + len(Rest1_S3N2_R3)
            print'จำนวนโหนดที่มีในครั้ง 3 =', P2
            ...
            Rest2_S3N2_inter_R2 = interintra2(Rest2_S3N2_inter_R1, Rest2_S3N2_R2)  # คำนวน รnterintra รอบที่ 3
            # -------------- รอบที่ 4 -----------------------
            Rest2_S3N2_R3 = Next_SubN2N1(Rest2_S3N2_inter_R2, Sub3)  # หา Sub ข้างเคียงใหม่อีกครั้ง รอบที่ 4
            print'Sub จากการหาโหนดเหมือนกัน 2 รอบที่ 3=', len(Rest2_S3N2_R3)  # พิมพ์จำนวน sub ข้างเคียงรอบที่ 4
            Rest2_S3N2_inter_R3 = interintra2(Rest2_S3N2_inter_R2, Rest2_S3N2_R3)  # คำนวน รnterintra รอบที่ 4
            ...
            if len(Rest2_S3N2_R1) or len(Rest2_S3N2_R2) == 0:  # เมื่อไม่เจอ sub รอบๆแล้ว ให้มาขึ้น Cluster ใหม่
                ...
                print'จำนวนโหนดที่มีในครั้ง 4 =', P3
                Q3 = Number_of_nodes - P3
                ...
                #print'Rest3_S3N2_inter_R1 =', Rest3_S3N2_inter_R1
                # -------------- รอบที่ 3 -----------------------
                ...
                Rest3_S3N2_inter_R2 = interintra2(Rest3_S3N2_inter_R1, Rest3_S3N2_R2)  # คำนวน รnterintra รอบที่ 3
                # -------------- รอบที่ 4 -----------------------
                Rest3_S3N2_R3 = Next_SubN2N1(Rest3_S3N2_inter_R2, Sub3)  # หา Sub ข้างเคียงใหม่อีกครั้ง รอบที่ 4
                print'Sub จากการหาโหนดเหมือนกัน 2 รอบที่ 3=', len(Rest3_S3N2_R3)  # พิมพ์จำนวน sub ข้างเคียงรอบที่ 4
                Rest3_S3N2_inter_R3 = interintra2(Rest3_S3N2_inter_R2, Rest3_S3N2_R3)  # คำนวน รnterintra รอบที่ 4
                # -------------- รอบที่ 5 -----------------------
                Rest3_S3N2_R4 = Next_SubN2N1(Rest3_S3N2_inter_R3, Sub3)  # หา Sub ข้างเคียงใหม่อีกครั้ง รอบที่ 4
                print'Sub จากการหาโหนดเหมือนกัน 2 รอบที่ 3=', len(Rest3_S3N2_R4)  # พิมพ์จำนวน sub ข้างเคียงรอบที่ 4
                Rest3_S3N2_inter_R4 = interintra2(Rest3_S3N2_inter_R3, Rest3_S3N2_R4)  # คำนวน รnterintra รอบที่ 4

                if len(Rest3_S3N2_R1) or len(Rest3_S3N2_R2) or len(Rest3_S3N2_R3) or len(Rest3_S3N2_R4)== 0:  # เมื่อไม่เจอ sub รอบๆแล้ว ให้มาขึ้น Cluster ใหม่
                    Cluster5 = Rest3_S3N2_inter_R3  # list[.,.,.] นับไม่ได้ ไว้สร้างกราฟ
                    print'โหนดทั้งหมดที่ใช้ในการหาครั้ง 5 =', Cluster5
                    P4 = len(Rest3_S3N2_R0) + len(Rest3_S3N2_R1) + len(Rest3_S3N2_R2)+len(Rest3_S3N2_R3)+len(Rest3_S3N2_R4)
                    print'จำนวนโหนดที่มีในครั้ง 2 =', P4
                    Q4 = Number_of_nodes - P4
                    print'จำนวนโหนดที่เหลืออยู่ในกราฟ', Q4, 'โหนด'

ClusterA = Cluster1+Cluster2+Cluster3+Cluster4+Cluster5
...
print'จำนวน Sub ที่เหลือจากการใช้ครั้ง 2 =', len(Start_Cluster2), 'Cycles'
Start_Cluster2_Sorted = sorted(Start_Cluster2)  # จัดเรียงซับที่เหลือจาก Cluster1
C2_inter_R0 = interintra(Start_Cluster2, Start_Cluster2_Sorted)  # เอาซับที่เหลือหา interintra
print'C2_inter_R0 =', C2_inter_R0 # list สร้างกราฟได้
...
Cluster5_M = Cluster5
C2_M = C2_inter_R0
Node_BetweenC = FindNodesBetweenCluster(Cluster1_M, Cluster2_M, Cluster3_M, Cluster4_M,Cluster5_M, C2_M, float(len(G.edges)))
# --------------Draw Graph-----------------
G = nx.Graph()
